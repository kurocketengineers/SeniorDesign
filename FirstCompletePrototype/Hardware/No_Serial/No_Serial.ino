...
static uint16_t item = 0;
static uint16_t secondChute = 3000;
    ...
    if (Handler->handle == bleRead.getValueAttribute().getHandle()) {
        ...
        */
        ...
        if (command == "arm")
        {
            ...
            //use accelerometer to detect launch
            while(!launch){
              int acceleration = 0;
              
              if(acceler.getXValue(acceleration)){
                if(acceleration > 20000){
                  digitalWrite(1, HIGH);
                  launch = true;
                }
            ...
            while(entry < ITEM_SIZE)
            {
                ...
                if (event.pressure)
                {
                    ...
                    */
                    ...
                    altitude = bmp.pressureToAltitude(seaLevelPressure, pressure.input);
                    
                    //DETECT APOGEE, Turn on other led
                    if(apogee == false){
                    ...
                    //2ND CHUTE, TURN OFF 1st LED 3000FT?
                    else{  
                      if( altitude < secondChute){
                        digitalWrite(0, LOW);
                      } 
                    }
            ...
            //END DATA COLLECTION TURN OFF 2ND LED 
            digitalWrite(1, LOW);
        ...
        else if (command == "read")
        {
          ...
          tick.attach(periodicCallback, .05);
          digitalWrite(0, LOW);
          digitalWrite(1, LOW);
        ...
        {
          uint8_t buffSet[TXRX_SIZE];
          uint16_t bytesReadSet, indexSet;
          String number = "";
          char num;
          //float p;

          //UPDATE secondChute
          while (Handler->handle != bleRead.getValueAttribute().getHandle());
          
          ble.readCharacteristicValue(bleRead.getValueAttribute().getHandle(), buffSet, &bytesReadSet);
          ...
          //set secondChute to that number
          secondChute = number.toInt();
        }
    }
...
void setup() {
    ...
    //accelerometer setup
    acceler.setPowerStatus(LR_POWER_NORM);
    acceler.setXEnable(true);
